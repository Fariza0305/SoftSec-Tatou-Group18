#!/bin/bash
# Regression Test: rt_path_traversal_upload_windows
# Description: Test path traversal protection (Windows style)
# Generated: 2025-10-17T18:58:26.234851

BASE_URL="${1:-http://localhost:5000}"
TEST_NAME="rt_path_traversal_upload_windows"

echo "Running regression test: $TEST_NAME"
echo "Description: Test path traversal protection (Windows style)"
echo ""
...
RESPONSE=$(curl -s -X POST "$BASE_URL/api/upload-document" \
    ...
    -F "file=@/dev/null;filename=..\..\..\windows\system32" 2>/dev/null)
